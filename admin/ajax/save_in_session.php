...
if(isset($_SESSION['cart'])){
    ...
    $check_availabe = 0;
    $check_availabe = check_duplicate_product($companyName, $diameter, $tireSize, $weight);
    $available_qty = 0;
    $check_the_qty = 0;

    if($check_availabe == 0){
        $available_qty = check_qty($companyName, $diameter, $tireSize, $weight, $conn);
        if($available_qty >= $quantity){
            $b = array("company_name" => $companyName, "diameter" => $diameter, "tire_size" => $tireSize, "weight" => $weight, "price" => $unitPrice, "qty" => $quantity);
        ...
        $av_qty = 0;
        $exist_qty = 0;
        $exist_qty = check_the_qty($companyName, $diameter, $tireSize, $weight);
    ...
    $available_qty = check_qty($companyName, $diameter, $tireSize, $weight, $conn);
    if($available_qty >= $quantity){
        $_SESSION['cart'] = array(array("company_name" => $companyName, "diameter" => $diameter, "tire_size" => $tireSize, "weight" => $weight, "price" => $unitPrice, "qty" => $quantity));
    }else{
        echo "Entered Quantity is not Available";
    }
}
